...
class Readability
{
	...
	**/ 
	public $regexps = array(
		'unlikelyCandidates' => '/cross_linking|prodinfo|most|bar|relation|tag|reporter|banner|next|prev|bottom|newslist|list|crumb|crumbs|terkait|author|social|bookmark|related|footer|foot|screen|copyright|combx|comment|community|date|disqus|extra|foot|header|menu|remark|rss|shoutbox|sidebar|sponsor|ad-break|advertise|iklan|agegate|pagination|pager|popup/i',
		'unlikelyCandidatesSrc' => '/banner|advertise|prev|next/i',
		'unlikelyCandidatesOnclick' => '/ga/i',
		'unlikelyCandidatesTarget' => '/_blank/i',
		'okMaybeItsACandidate' => '/and|description|thumb|article|body|column|main|img|shadow/i',
		'positive' => '/description|article|entries|body|img|content|entry|hentry|main|page|attachment|post|text|blog|story/i',
		'negative' => '/combx|comment|com-|contact|foot|date|tanggal|share|footer|_nav|footnote|masthead|media|meta|outbrain|promo|related|scroll|shoutbox|sidebar|sponsor|shopping|tags|tool|widget/i',
		'divToPElements' => '/<(a|blockquote|dl|div|article|img|ol|p|pre|table|ul)/i',
		'replaceBrs' => '/(<br[^>]*>[ \n\r\t]*){2,}/i',
		'replaceFonts' => '/<(\/?)font[^>]*>/i',	
		'normalize' => '/\s{2,}/',
		'killBreaks' => '/(<br\s*\/?>(\s|&nbsp;?)*){1,}/',
		...
		'html5tag'=> 'section|header|article|footer'
	);	
	...
	*/	
	function __construct($html, $url=null, $parser='libxml')
	{
		...
		/* HTML 5 Hack */
		$html5tag = explode("|",$this->regexps['html5tag']);
		foreach($html5tag as $t) {
			$html = str_ireplace(array('<'.$t,'</'.$t),array('<div','</div'),$html);
		}		
			
		/* Turn Off Link */
		if($this->preventLink) {
			$html = preg_replace($this->regexps['removeLink'],'\1',$html);
		} 
	...
	**/
	public function init()
	{
		...
		if (!$articleContent) {
			...
			$articleContent->innerHTML = '<p>Sorry, We was unable to parse this page for content.</p>';		
		}else{
			//Try Clean Unlikely Again
			$trydom = new DOMDocument;
			$trydom->preserveWhiteSpace = false;
			@$trydom->loadHTML(mb_convert_encoding($articleContent->innerHTML, 'HTML-ENTITIES', 'UTF-8'));			
			foreach($trydom->getElementsByTagName('*') as $node) {
				$tagName = strtoupper($node->tagName);
				$unlikelyMatchString = $node->getAttribute("class") . $node->getAttribute("id") . $node->getAttribute("itemtype");				
				if (
				...
				if($tagName == 'IMG') {
					$data_orig = @$node->getAttribute("data-original");
					if($data_orig) {
						$node->setAttribute("src",$data_orig);
					}
				}
				
				//Clear if empty content
				#if($tagName == 'P' && trim($node->textContent)=='') $node->parentNode->removeChild($node);
				
			}
			$html = $trydom->saveHTML();	
			//Remove comment line
	...
	*/
	function prepArticle($articleContent) {
		...
		$this->cleanConditionally($articleContent, 'h1');
		...
		***/
		...
		$this->cleanConditionally($articleContent, 'iframe');
		$this->cleanConditionally($articleContent, 'noscript');
	...
	protected function grabArticle($page=null) {
		...
		**/
		...
		//for ($nodeIndex=$targetList->length-1; $nodeIndex >= 0; $nodeIndex--) {
			...
			$regx = $this->regexps['unlikelyCandidates'];
			#$this->dbg("Find $regx => class='$class' id='$id'");
			#$this->dbg("Strip : $stripUnlikelyCandidates");
			...
			/* Remove unlikely candidates Src & Href */
			$unlikelyMatchString = @$node->getAttribute('src') . @$node->getAttribute('href');
			if($unlikelyMatchString) {
				if (					
					$tagName != 'BODY' && 
					preg_match($this->regexps['unlikelyCandidatesSrc'], $unlikelyMatchString)
				)
				{					
					$this->dbg('Removing unlikely candidate src & href - ' . $unlikelyMatchString);
					$node->parentNode->removeChild($node);
					$nodeIndex--;
					continue;
				}
			...
			/* Remove unlikely candidates OnClick */
			$unlikelyMatchString = @$node->getAttribute('onclick') . @$node->getAttribute('onClick');
			if($unlikelyMatchString) {
				if (					
					$tagName != 'BODY' && 
					preg_match($this->regexps['unlikelyCandidatesOnclick'], $unlikelyMatchString)
				)
				{					
					$this->dbg('Removing unlikely candidate onClick - ' . $unlikelyMatchString);
					$node->parentNode->removeChild($node);
					$nodeIndex--;
					continue;
				}
			...
			/* Remove unlikely candidates target */
			$unlikelyMatchString = @$node->getAttribute('target');
			if($unlikelyMatchString) {
				if (					
					$tagName != 'BODY' && 
					preg_match($this->regexps['unlikelyCandidatesTarget'], $unlikelyMatchString)
				)
				{					
					$this->dbg('Removing unlikely candidate Target - ' . $unlikelyMatchString);
					$node->parentNode->removeChild($node);
					$nodeIndex--;
					continue;
				}
		...
		**/
		...
		for ($pt=0; $pt < count($nodesToScore); $pt++) {
			...
			$htm = $this->getInnerHTML($parentNode);
			$doc = new DOMDocument();
 			$doc->registerNodeClass('DOMElement', 'JSLikeHTMLElement');
			@$doc->loadHTML($htm);
			$imgcount = $doc->getElementsByTagName("img")->length;
			#$this->dbg("Images Count : $imgcount");
			if(strlen($innerText) < 25 && $imgcount==0) {					
				continue;
			}
	...
	**/
	...
	public function getInnerHTML($e, $normalizeSpaces=true) {
		return $e->innerHTML;
	}
			...
			if ($weight + $contentScore < 0) {
				...
				**/
				...
				if ($this->lightClean) {
					...
					if ( ($img > $p) && ($img > 15) ) {
						$this->dbg(' more than 15 images and more image elements than paragraph elements');
						$toRemove = true;
						...
						$toRemove = true; 
					} else if ($contentLength < 5 && ($embedCount === 0 && ($img === 0 || $img > 2))) {
						$this->dbg(' content length less than 5 chars, 0 embeds and either 0 images or more than 2 images : '.$tagsList->item($i));
						$toRemove = true;
					} else if($weight < 5 && $linkDensity > 0.2) {
						$this->dbg(' weight smaller than 5 and link density above 0.2');
						$toRemove = true;
					} else if($a > 2 && ($weight >= 5 && $linkDensity > 0.5)) {
						$this->dbg(' more than 2 links and weight above 5 but link density greater than 0.5');
						$toRemove = true;
